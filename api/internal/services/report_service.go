...
func (service ReportService) buildModel(
	...
	resolveCurrentViewerPaidBy(&filter, userId)
...
// currentViewerPaidBySentinel is the paid-by filter value the desktop report builder
// stores for its "Current viewer (me)" option (desktop CURRENT_VIEWER_PAID_BY_ID). It
// is negative so it can never collide with a real (positive) user id.
const currentViewerPaidBySentinel = -1

// resolveCurrentViewerPaidBy substitutes the "current viewer" sentinel in the paid-by
...
func resolveCurrentViewerPaidBy(filter *commands.ReceiptPagedRequestFilter, userId uint) {
	...
	for _, value := range values {
		if id, isNumber := value.(float64); isNumber && id == currentViewerPaidBySentinel {
