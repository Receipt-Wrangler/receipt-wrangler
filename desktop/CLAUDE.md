...
## Resizable image pane (receipt form)

The receipt form's form/image row is split by a draggable vertical divider, so the image can be
widened for a receipt that is hard to read. It replaced a **Collapse / Expand** icon button that only
ever toggled `col-3` on the carousel wrapper — the panes themselves were bare Bootstrap `.col`
siblings, i.e. a hard 50/50 that nothing could change.

- **`app-pane-splitter`** (`src/shared-ui/pane-splitter/`, standalone, registered in `SharedUiModule`'s
  `imports` + `exports`) is generic and presentational: it reports the width of the pane on its right
  as a percentage of `[container]` and writes that to the container as the CSS custom property
  **`--rw-pane-ratio`** (exported as `PANE_RATIO_PROPERTY`). The container's own stylesheet decides
  what to do with it — nothing about receipts is baked into the component.
- **Deliberately NOT `cdkDrag`.** cdkDrag resizes nothing; it *translates the element it sits on*,
  which fights a splitter the layout is already moving as the ratio changes (you get double movement,
  or a `reset()`-per-frame jitter). `pointerdown` → `setPointerCapture` → `pointermove` → `pointerup`
  keeps tracking when the cursor leaves the handle and covers mouse/touch/pen in one path. It also
  keeps the splitter off the same drag system the image pan (`cdkDrag` on the `<img>` in
  `app-image-viewer`) already uses.
- **`pointermove` is a native listener, not an Angular host binding.** A host binding schedules change
  detection on *every* event, and the receipt form is a ~450-line template whose children are not
  `OnPush` — a 120Hz drag would re-render the whole page. The drag writes the container's style
  directly via `Renderer2` (with `RendererStyleFlags2.DashCase`, which custom properties need) and
  commits to the `ratio` model **once, on release**. An `effect()` mirrors the signal back onto the
  container, which is what covers the initial paint and keyboard moves.
- **The ratio is recomputed from where the drag started**, never accumulated from the previous move,
  so it cannot drift and re-clamping every move is idempotent.
- **Two clamps, and the pixel one is load-bearing.** `min`/`max` (20/80) bound the percentage, but
  `minLeftPaneWidth` (360px) additionally lowers the effective max: Material's form-field infix is
  180px and the form pane puts an `app-input` beside an `app-checkbox`, so a percentage floor alone
  lets the pane collapse until its fields overflow instead of pushing back.
- **a11y:** `role="separator"` **plus an explicit `aria-orientation="vertical"`** — a separator
  defaults to *horizontal*, which is the wrong axis here. Arrows move ±2 points, Home/End jump to the
  bounds, double-click resets to `defaultRatio`.

**Layout** (`receipt-form.component.scss`, previously an empty file):

- The image pane is the only one with a percentage — `flex: 0 0 var(--rw-pane-ratio, 50%)` — and the
  form pane is `flex: 1 1 0`, so the splitter's own width always comes out of the form pane and the
  row cannot overflow.
- **`gap: 0`, and the splitter is itself `1.5rem` wide.** That reproduces the Bootstrap gutter it
  replaced (0.75rem of `.row > *` padding each side) exactly. A `gap` would apply on *both* sides of
  the splitter and widen the gutter to ~58px.
- **`min-width: 0` on both panes is mandatory, not polish.** `.row > *` used to cap them at
  `max-width: 100%`; without that a large receipt image gives its pane a min-content width of the
  image's intrinsic size, which floors the flex basis and silently overruns the ratio.
- **Below 768px the panes stack and the splitter is hidden.** Dropping `.row` also dropped its
  `flex-wrap: wrap`, which was the only thing that ever let them break onto separate lines.
- **Every rule is scoped under `.receipt-panes`, and must stay that way.** `#expandedImageTemplate`
  (the fullscreen dialog) is declared in this component, so its embedded view carries this
  component's encapsulation attribute even while it renders inside the MatDialog overlay — an
  unscoped rule matching `app-carousel` or its descendants would reach into fullscreen. (The same
  trap in the other direction: `CarouselComponent` is `ViewEncapsulation.None`, so everything in
  `carousel.component.scss` is global.)

**The `showLargeImagePreviews` user preference now seeds the starting width** (40% off / 65% on)
rather than picking between two presets, so the Settings checkbox stays meaningful with the button
gone. Note "off" used to render a ~12.5% thumbnail, so 40% is a deliberate enlargement — the image
filling its pane is the new baseline and the drag adjusts from there.

**Seed it in a field initializer, never in the `activatedRoute.data` subscription.** That
subscription re-fires on every route-data emission, including every `queueNext()`/`queuePrevious()`,
so seeding there would discard a width the user had just dragged each time they stepped to the next
receipt. Pinned by a spec case.

The width is **session-only** by design — no NGXS slice, no localStorage. `app-image-viewer` and
`src/carousel/` are untouched, so pan, zoom and the fullscreen dialog behave exactly as before.
Known limitation: the pan is a pixel `translate3d` independent of pane width, so resizing a pane whose
image was already panned leaves the same pixel offset (pre-existing, but continuous dragging makes it
much more visible).
