...
void main() {
  ...
  group('buildCustomFieldValues', () {
    test('keeps each entry and refreshes it from the form', () {
      final values = buildCustomFieldValues(
        attachedValues: [
          buildCustomFieldValue(
            customFieldId: _textId,
            id: 77,
            receiptId: _receiptId,
            stringValue: 'stale',
          ),
      ...
      expect(values.single.id, 77, reason: 'the existing row is preserved');
      expect(values.single.receiptId, _receiptId);
      expect(values.single.stringValue, 'fresh');
    });

    test('keeps an emptied value attached with a null column', () {
      final values = buildCustomFieldValues(
        attachedValues: [
          buildCustomFieldValue(customFieldId: _textId, stringValue: 'PO-1234'),
        ],
      ...
      expect(values.single.customFieldId, _textId);
      expect(values.single.stringValue, isNull);
    });
    ...
    test('falls back to the stored value when the template is missing', () {
      ...
      expect(values.single.stringValue, 'PO-1234');
    });
  });
}
