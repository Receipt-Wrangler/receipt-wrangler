...
class _GroupAppBar extends State<GroupAppBar> {
  ...
  Widget build(BuildContext context) {
    ...
    // The filter action and the receipt-entry overflow belong to the receipts
    // screen only. This app bar is shared with the dashboards route, so gate on
    // the route itself -- the same check main.dart uses to pick that route's
    // padding.
    ...
    return TopAppBar(
      ...
      actions: isReceiptsRoute
          ? const [ReceiptFilterButton(), ReceiptEntryOverflowMenu()]
          : null,
    );
  }
}
